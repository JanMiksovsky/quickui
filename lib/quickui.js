        ...
        if ( json ) {
            ...
            newClass.prototype.extend({
                ...
                render: function render() {
                    superclass( this )
                        .render()               // Superclass renders first.
                        .json( properties );    // Apply JSON using super's setters.
                    return this;
                },
...
$.extend( Control.prototype, {
    ...
    json: function( json, logicalParent ) {
        for ( var i = 0; i < this.length; i++ ) {
            ...
            var result = applyControlJson( control, json, logicalParent || control );
            if ( !result || result.length !== 1 || result[0] !== control[0] ) {
                // JSON returned something other than the element we started with,
                // which means its last key specified a getter. Return that result. 
                return result; 
            }
        }
        return this;
...
/*
 * Apply the given JSON to the indicated single control.
 */
function applyControlJson( control, json, logicalParent ) {
    var result = control;
    for ( var key in json ) {
        if ( result[ key ] === undefined ) {
            var message = "JSON key \"" + key + ":\" is not defined for control class \"" + result.className() + "\".";
            throw message;
        }
        var value = evaluateControlJsonValue( json[ key ], logicalParent );
        result = result[ key ].call( result, value );
    }
    return result;
}
...
function evaluateControlJson( json, logicalParent ) {
    ...
    var properties = copyExcludingKeys( json, reservedKeys );
    var result = control.json( properties, logicalParent );
