...
$.extend( Control, {
    ...
    classHierarchy: function( startingClass ) {
        ...
        if ( !( controlClass && controlClass.prototype && controlClass.className ) ) {
            return "";
        }
        if ( !controlClass._classHierarchy )          // Already have memoized result?
        {
            var result = controlClass.className;
     ...
     * creates a "name" property that's unchangeable and undeleteable.
     */
    className: "Control",
    ...
    subclass: function( className, renderFunction, tag ) {
        ...
        // $.sub uses $.extend to copy properties from super to subclass;
    ...
    subclass2: function( json ) {
        ...
        newClass.extend({
            ...
            // TODO: Replace classHierarchy() with simple reference to _classHierarchy
            _classHierarchy: json.name + " " + superclass.classHierarchy(), // superclass._classHierarchy

            subclass2: superclass.subclass2 // So the new class can itself be subclassed.
        });
                ...
                for ( var property in json ) {
                    if ( property !== "name" && property !== "tag" ) {
                        ...
                        for ( var i = 0; i < this.length; i++ ) {
                            var logicalParent = this.nth(i);
                            var value = valueFromJson( object, logicalParent );
                            $super.nth(i)[ property ]( value );
                        }
                    }
                }
                return this;
    ...
    // TODO: Rename this to classHierarchy
    _classHierarchy: "Control",
...
function controlFromJson( json, logicalParent ) {
    ...
    if ( isHtml ) {
        ...
        var classFn = window[ json.control ];
