...
jQuery.extend(Control, {
    ...
    createAt: function(target, properties) {
        ...
        $control.addClass($control.classHierarchy(this));
	...
	subclass: function(className, renderFunction) {
		var superClass = this;
		...
		newClass.fn.className = className;
		newClass.fn.render = function() {
		    superClass.fn.render.call(this);
			if (renderFunction)
			{
...
jQuery.extend(Control.ElementPropertyFactory.prototype, {
    ...
    content: function(setterFunction) {
        return this._elementProperty(
            function($element, value) {
                return Control($element).content(value);
            },
            setterFunction);
    },
