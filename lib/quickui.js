...
function significantContents( element ) {
    ...
    // Use $.contents (with an "s") to get children including text nodes.
    ...
    for ( var i = 0, length = contents.length; i < length; i++ ) {
        ...
        switch ( content.nodeType ) {
            case 3:                     // Text node
                var s = $.trim( content.nodeValue );
                if ( s.length > 0 ) {
                    return s;           // Found significant text; return that
                }
                break;
            case 8:                     // Comment nodes don't count
                break;
            default:    
                return contents;        // Found some real element
        }
    }
