...
$.extend( Control, {
    ...
    createAt: function( target, properties ) {
        ...
        var oldContents = $controls.map( function( index, element ) {
            var content = $( element ).contents(); // Standard content*s* function.
            // Don't return an empty array or text which is purely whitespace.
            var isContentEmpty = ( content.length === 0 )
                || ( content.length === 1
                    && content[0].nodeType === 3
                    && $.trim( content[0].nodeValue ).length === 0 );
            return !isContentEmpty ? content : null;
        }).get();
