...
$.event.special.layout = {
    ...
    handle: function( event ) {
        ...
        var element = $( this );
        if ( !Control( element ).inDocument() ) {
            // Not currently in document; don't handle the event.
            return;
        }
        
        // Compare element's current size with its previously recorded size.
        var previousSize = element.data( "_size" ) || {};
        var size = {
            height: element.height(),
            width: element.width()
        };
        if ( size.height === previousSize.height && size.width === previousSize.width ) {
            // Size hasn't actually changed; don't handle the event.
            return;
        }
        
        // Record the new size and invoke the handler.
        element.data( "_size", size );
