...
$.extend( Control, {
    ...
    createAt: function( target, properties ) {
        ...
        var defaultTarget = "<" + this.prototype.tag + "/>";
        ...
        if ( target === null ) {
            ...
            var existingTag = $controls[0].nodeName.toLowerCase();
            if ( existingTag !== this.prototype.tag.toLowerCase()
                && existingTag !== "body" ) {
        ...
        properties = ( typeof properties === "string" )
            ? { content: properties }   // Plain string becomes control content
            : properties;
    ...
    subclass: function( json ) {
        ...
        if ( json ) {
             ...
             * "settings" on the class' prototype for use by render(). 
             */
            var settings = copyExcludingKeys( json, {
                className: true,
                genericSupport: true,
                prototype: true,
                tag: true
            ...
            newClass.prototype.settings = null;
        }
    ...
    _replaceElements: function( $existing, $replacement ) {
        ...
        var ids = $existing.map( function( index, element ) {
            return $( element ).prop( "id" );
        });
        var $new = $replacement.replaceAll( $existing );
        // Put IDs onto new elements.
        $new.each( function( index, element ) {
            $( element ).prop( "id", ids[ index ] );
        });
        return $new;
    },
...
$.extend( Control.prototype, {
     ...
     * starting at the *top*. Each class' settings are passed to property
     * setters on that class' superclass. That is, each class defines itself in
     * the semantics of its superclass.
     */    
    render: function render() {
        var classFn = this.constructor;
        if ( classFn !== Control ) {
    ...
    transmute: function( newClass, preserveContent, preserveClasses, preserveEvents ) {
        ...
        var $controls = classFn.createAt( this );
    ...
    _controlClass: function( classFn ) {
        return classFn
            ? this.data( Control._controlClassData, classFn )
            : this.data( Control._controlClassData );
    },
