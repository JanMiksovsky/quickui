...
jQuery.extend( Control, {
    ...
    bindTo: function() {
        ...
        return function binding( value ) {
            ...
            for ( var i = 0, length = functionNames.length; i < length; i++ ) {
                ...
                if ( fn === undefined ) {
                    var message = "Control class \"" + this.className + "\" tried to bind to undefined getter/setter function \"" + functionNames[i] + "\".";
                    throw message;
                }
    ...
    // Controls waiting (via an insertedIntoDocument call) to be added to the body.
    ...
    // True if Control is currently listening for DOMNodeInserted events.
...
jQuery.extend( Control.prototype, {
     ...
     *
     * If the culture of a control is changed, a _cultureChanged() method will
     * be invoked on the control, which can then update as appropriate.
     * The _cultureChanged() method should not be invoked directly.
     */
     ...
     * function via this.genericIfClassIf(Bar).
     */
