...
class window.Control2 extends jQuery
  constructor: ( selector, context ) ->
    ...
    # Figure out which class is being instantiated. We can't just look at "this",
    # because jQuery supports a static constructor form: i.e., you can call
    # jQuery(...) without "new", in which case "this" will be the window. We
    # can't just look at the current function, because CoffeeScript doesn't
    # (yet?) support named functions. So we walk up the call tree to figure out
    # which function was actually called. This entails using the deprecated
    # "caller" value, which is not allowed in strict mode.
    classFn = arguments.callee

    caller = classFn.caller
    if ( caller:: ) instanceof classFn and caller.__super__ isnt undefined
      ...
      # constructor generated by CoffeeScript, as QuickUI controls shouldn't do
      # real work in their constructor; they should use initialize() instead.
      # We want to instantiate the subclass instead of the general Control class.
      classFn = caller

      # Ensure the class has been treated to be jQuery- and QuickUI-compatible.
      if not isCompatible classFn
        makeCompatible classFn

    # Actually create an instance of the init helper class per standard jQuery.
    new classFn::init selector, context
